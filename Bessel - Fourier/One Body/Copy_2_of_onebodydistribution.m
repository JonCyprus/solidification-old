...
T = 1000;               % temperature (K), regulates diffusion term, changes
...
L =  (38.9823 * Re)/2;       % Length of simulation cell edge %Changed to half the size for compatability
delta = L/n;                % Spacing of the grid
dA = delta * delta;         % Area between grid spacings
N =  1700/4;               % number of particles removed 4 * (division by 16 from original)
...
%p0_12 = circshift(p0_12,[n/2,n/2]); commented out
...
for s = start:total_step 
    % New terms
    c_0 = sum(sum((p0_12 .* p1 .* p2 * dA))) / (one_body^2);
    c_1 = (N * (N - 1) ./ c_0);
    c1_andp0_12 = p0_12 .* c_1;
    c1_andp0_12 =circshift(c1_andp0_12, [n/2,n/2]);
    ...
    grad_p0_12_x = nablax(c1_andp0_12, delta);
    grad_p0_12_y = nablay(c1_andp0_12, delta);

    % Integral part 1
    wx1 = grad_v2_12_x .* grad_p0_12_x;
    wy1 = grad_v2_12_y .* grad_p0_12_y;
    fw1 = fft2(lap_v2_12 .* c1_andp0_12 + wx1 + wy1);

    % Integral part 2
    wx2 = (grad_v2_12_x .* c1_andp0_12);
    fwx2 = fft2( wx2 );
    wy2 = (grad_v2_12_y .* c1_andp0_12);
    fwy2 = fft2 ( wy2 );
